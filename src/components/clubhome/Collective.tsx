  ...
  return (
    <div className="relative mx-auto aspect-[4/3] w-full max-w-[360px]">
      <svg viewBox="0 0 100 100" className="absolute inset-0 h-full w-full" aria-hidden>
        ...
        <circle cx={cx} cy={cy} r="9" fill="var(--color-teal-400)" opacity="0.12" />
        <circle cx={cx} cy={cy} r="9" fill="none" stroke="var(--color-teal-500)" strokeWidth="0.8" />
      </svg>
      ...
      >
        <span className="font-display text-lg font-black leading-none text-teal-600">∞</span>
      </div>
      ...
      {positions.map((p, i) => (
        ...
        >
          {!decorative && p.name ? (
            ...
            <span
              className="block h-3.5 w-3.5 rounded-full bg-teal-400 ring-2 ring-card"
    ...
    <div className="mt-5 grid grid-cols-2 divide-x divide-y divide-sand border-t border-sand sm:grid-cols-5 sm:divide-y-0">
      {rows.map((r) => (
        <div key={r.label} className="px-3 py-3 first:pl-0">
          ...
          <div className="text-[11px] font-medium text-soft">{r.label}</div>
        </div>
      ))}
    </div>
  );
...
export default function Collective({
  ...
  if (!floorMet) {
    return (
      <section aria-label="The Collective — founding era" className="rounded-2xl border border-teal-500/25 bg-teal-400/[0.05] p-5 sm:p-7">
  ...
  return (
    <section aria-label="The Collective" className="rounded-2xl border border-sand bg-card p-5 shadow-soft sm:p-7">
      <SectionLabel tone="teal" live liveTone="teal">
        The Collective
      </SectionLabel>
      <div className="mt-4 grid items-center gap-6 lg:grid-cols-[1fr_1.1fr]">
        <div>
          <div className="font-display text-5xl font-black leading-none tracking-tight text-ink sm:text-6xl">
            {collective!.actionsToday.toLocaleString()}
          </div>
