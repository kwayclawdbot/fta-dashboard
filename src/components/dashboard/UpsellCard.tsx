 ...
 * names exactly what's behind the door, then offers the forward path (join FIC).
 *
 * Variants:
...
const CONTEXT: Record<UpsellContext, ContextCopy> = {
  generic: {
    ...
    body: "Join the Family Investing Club to unlock the full experience — the whole family, one membership.",
    cta: "Join FIC — $99/mo",
  },
  simulator: {
    icon: LineChart,
  ...
  watchlist: {
    ...
    cta: "Unlock the watchlist — join FIC",
  },
  missions: {
    icon: Target,
  ...
  flashcards: {
    ...
    cta: "Unlock flashcards — join FIC",
  },
  live: {
    icon: Video,
    ...
    cta: "Join the live classes — join FIC",
  },
  progress: {
    icon: Trophy,
    ...
    cta: "Unlock your progress — join FIC",
  },
  "trend-or-trap": {
    icon: Gamepad2,
    eyebrow: "A members' game",
    title: "Trend or Trap",
    body: "Read the pattern and call it — is price really trending, or is it a trap? A fast, addictive way to train your eye. FIC members play the full set of practice games; Candle Battle is yours to play free right now.",
    cta: "Play the full set — join FIC",
  },
  courses: {
    icon: BookOpen,
    ...
    cta: "Unlock every lesson — join FIC",
  },
  lesson: {
    icon: BookOpen,
  ...
  pick: {
    ...
    cta: "Unlock the thesis — join FIC",
  },
  "pick-engage": {
    icon: MessageCircle,
    ...
    cta: "Join FIC to comment",
  },
  "fic-club": {
    icon: MessageCircle,
    eyebrow: "The members' room",
    title: "FIC Club chat",
    body: "The members' room is where families talk shop all week. You're welcome in the Free Lounge anytime — join FIC to jump into the club.",
    cta: "Join the club — join FIC",
  },
  community: {
    icon: MessageCircle,
    eyebrow: "You're viewing as a free member",
    title: "Post, like, and comment",
    body: "Join FIC to share wins, ask questions, and talk with the community — not just read along.",
    cta: "Join FIC",
  },
  "journey-final": {
    icon: Sparkles,
    eyebrow: "You've seen what's inside",
    title: "Unlock everything — join FIC",
...
export default function UpsellCard({
  ...
  return (
    <div className={`max-w-lg mx-auto py-6 ${className}`}>
      ...
      >
        <div className="w-14 h-14 mx-auto rounded-2xl bg-gold-400/15 flex items-center justify-center mb-4 relative">
