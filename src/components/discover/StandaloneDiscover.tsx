  ...
  return (
    <Link href={`/research/${encodeURIComponent(row.ticker)}`} className="flex items-center gap-2.5 border-b border-[#2A2530] py-2.5 last:border-b-0">
      ...
      <div className="text-right">
        ...
        {row.watchers != null && <p className="mt-0.5 text-[8px] text-[#6E6774]">{row.watchers.toLocaleString()} watching</p>}
      </div>
    </Link>
  );
}
    ...
    <div className="cc-app-screen mx-auto max-w-[760px] px-[18px] pb-20 pt-[18px] sm:rounded-[26px] sm:border sm:border-[#2A2530]">
      <header className="flex items-start justify-between gap-4">
        ...
        <div className="flex gap-2">
          ...
          {showScreener ? (
            <Link href="/screener" className="cc-app-card grid h-[34px] w-[34px] place-items-center" aria-label="Open stock screener">
      ...
      <section className="mt-5">
        <div className="flex items-center justify-between">
          <h2 className="cc-app-signal text-[9.5px] font-semibold uppercase tracking-[.16em] text-[#F4F0EC]">Top in the club</h2>
      ...
      {movers.length > 0 && (
        <section className="mt-5">
          <div className="flex items-center justify-between">
            ...
            <Link href="/screener" className="text-[10.5px] font-bold text-[#FF9A4D]">See more ›</Link>
          </div>
      ...
      {lead && (
        <section className="mt-5 overflow-hidden rounded-[18px] border border-[#2E3A18] bg-[linear-gradient(125deg,#131A0A,#17141A_70%)] p-4">
          ...
          <h3 className="mt-2 text-[15px] font-bold text-[#F4F0EC]">{lead.ticker} holds the Club&apos;s highest verified attention rank.</h3>
          <p className="mt-2 text-[10.5px] text-[#8F8894]">
            {lead.sentiment?.bullPct != null ? `${lead.sentiment.bullPct}% of positioned members are bullish` : "Member sentiment is still forming"}
