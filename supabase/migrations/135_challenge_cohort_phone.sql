-- 135 — Surface phone in the challenge cohort dashboard (Lane C9b add-on)
--
-- The email-first register-email endpoint now captures name + phone (E.164
-- normalized, with an SMS-consent marker). This teaches admin_challenge_cohort
-- to return each member's phone so /admin/crm/challenge rows can show it. Pure
-- read-side change — same shape as migration 130 plus `phone` on the cohort CTE
-- and every member row. Everything else is unchanged.
...
begin
  ...
  select jsonb_build_object(
    ...
    'members', coalesce((
      ...
      from (
        select c.email, c.first_name, c.phone, c.created_at, c.user_id, c.onboarding_complete,
