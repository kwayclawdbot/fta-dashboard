...
> **⚠️ 2026-07-29 AMENDMENT — EXECUTION MODEL CHANGED (owner-approved).** Point 1 below originally said the shipped light system's *structure survives* and only tokens swap. That instruction produced repeated old/new design blends and is DEAD. The conversion now runs as a **strangler rebuild** on branch `conv/ui-v3`: screens are **translated near-verbatim from the artboard DOM** (mockups committed at `.planning/design-project-v2/mockups/`, artboards = `[data-screen-label]`, inline styles) into a fresh `src/ui-v3/` + `src/app/v3/` tree behind an **enforced ESLint import wall** (no old components, no old classNames), styled with tokens **generated** by `npm run tokens:v3`, and gated by **Playwright side-by-side screenshots vs the artboard**. Old screens are deleted at route cutover, never restyled. Unmocked screens compose from ui-v3 primitives + `DESIGN-GRAMMAR.md` only. Everything else in this plan (gap matrix, route mapping, track W, owner decisions) still stands.

1. **The mockup family already solves the light-vs-dark tension.** `App.dc` (dark) and `App Light.dc` are the SAME 23 screens in two themes. The shipped app already has a `data-theme` axis: dark canvas = dark theme, Light canvas = light theme — both emitted by the token extractor. ~~The 07-27 light redesign's structure survives; its token values are replaced.~~ **Superseded — see amendment above: nothing visual survives; structure and tokens both come from the mockup.**
